...
export class ExamComponent implements OnInit {
  ...
  private loadExams() {
    var studentId = {
      studentId: this.personLoginDetail.username
    }
    this.service.getExamSeat(studentId).then((result) => {
      this.items = result
      this.service.setExamList(result)
    }).catch((err) => {
  ...
  ngDoCheck(): void {
    this.items = this.service.examList
  }
